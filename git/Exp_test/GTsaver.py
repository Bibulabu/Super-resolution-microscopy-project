...
# RawSIMData_gt就是SIM raw 9-frame stack： 502x502x9
...
def save_gt(image_data):
    # image_data: 3D numpy array with shape (nx, ny, nz)

    for z in range(image_data.shape[2]):
        slice_i = image_data[:, :, z]
        slice_i = (slice_i - np.min(slice_i)) / (np.max(slice_i) - np.min(slice_i)) * 255
        image_slice = slice_i.astype(np.uint8)
        image_slice = Image.fromarray(image_slice)
        image_slice = image_slice.resize((512, 512))
        
        if z<10:
            image_slice.save('microtubules_gt/00{:d}.png'.format(z), format='png')
            print('00{:d} complete'.format(z))
        else:
            image_slice.save('microtubules_gt/0{:d}.png'.format(z), format='png')
            print('0{:d} complete'.format(z))
    
if __name__ == '__main__':
    _, data = read_mrc('GT_all.mrc')
    save_gt(data)
